...
test("Human units are not moved by CPU logic",()=>{let g=new Game(12),u=g.addUnit(Side.P1,Kind.PAWN,[0,0],{spend:false,ready:true});runStep(g,"human","defense");assert.deepEqual(u.pos,[0,0])});
...
test("DESTINY and START controls work in a browser-like DOM",()=>{
  ...
  assert(!elements.get("board").children.slice(-40).some(cell=>cell.classList.contains("deployable")),"only one manual deployment is allowed per round");
  elements.get("step").onclick();
  assert.equal(elements.get("time").textContent,3);
